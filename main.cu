...
int main(int argc, char** argv) {
    ...
    check_cuda_error(cudaSetDevice(0), "Unable to set device 0");
    init_fields<<<(nx * (ny / 2 + 1) + 255) / 256, 256>>>(nx, ny / 2, ez_0, dz_0, hx_0, hy_0, er_0, mh_0);
    cudaDeviceSynchronize();

    check_cuda_error(cudaSetDevice(1), "Unable to set device 1");
    init_fields<<<(nx * (ny / 2 + 1) + 255) / 256, 256>>>(nx, ny / 2, ez_1, dz_1, hx_1, hy_1, er_1, mh_1);
    cudaDeviceSynchronize();
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        check_cuda_error(cudaSetDevice(0), "Unable to set device 0");
        int total = nx * ny / 2;
        update_h_kernel<<<(nx * (ny / 2)  + 255) / 256, 256>>>(nx, ny / 2, dx, dy, ez_0, mh_0, hx_0, hy_0, total);
        update_e_kernel<<<(nx * (ny / 2)  + 255) / 256, 256>>>(nx, source_position, t, dx,dy, C0_p_dt, ez_0, dz_0, er_0, hx_0, hy_0, total);
        
        
        // GPU 1 update
        check_cuda_error(cudaSetDevice(1), "Unable to set device 1");
        update_h_kernel<<<(nx * (ny / 2)  + 255) / 256, 256>>>(nx, ny / 2, dx, dy, ez_1, mh_1, hx_1, hy_1, total);
        update_e_kernel<<<(nx * (ny / 2)  + 255) / 256, 256>>>(nx, source_position - (nx * (ny / 2)), t, dx,dy, C0_p_dt, ez_1, dz_1, er_1, hx_1, hy_1, total);
